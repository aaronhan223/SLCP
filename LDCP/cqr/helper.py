...
def run_icp(nc, X_train, y_train, X_test, idx_train, idx_cal, significance, local, k, condition=None):
    ...
    """
    icp = IcpRegressor(nc, local, k, significance=significance, condition=condition)
...
class Linear_RegressorAdapter(RegressorAdapter):
    ...
    def __init__(self,
        ...
        self.test_ratio = test_ratio
        self.random_state = random_state
        self.model = torch_models.lr_q_model(quantiles=self.all_quantiles, in_shape=in_shape)
        self.loss_func = torch_models.AllQuantileLoss(self.all_quantiles)
        self.learner = torch_models.LearnerOptimizedCrossing(self.model,
                                                             partial(learn_func, lr=lr, weight_decay=wd),
                                                             self.loss_func,
    ...
    def predict(self, x):
        ...
        """
        return self.learner.predict(x.astype(np.float32))
